# The Human Data Collective: A Standard for Open, Sustainable, and Equitable Machine Intelligence
...
**Figure i. Human Data Collective architecture**

```mermaid
flowchart TB
  creator[Creators] --> hdc((Human Data Collective))
  review[Peer Reviewers] --> hdc
  ops[Operations] -.-> hdc
  hdc -->|training data| frontier[Frontier<br/>branded token + premium]
  hdc -->|open access| oss[Open-source<br/>public-good token]
  frontier -. 1% share .-> hdc
  frontier -->|baseline| smart[Smart Contract<br/>carbon and water pool]
  oss -->|baseline| smart
  smart --> eco[Local Eco Remediation]
```

*Figure note. This diagram shows HDC's v1 operating loop at the role and settlement level. Creators and peer reviewers supply attested training data; frontier and open-source model paths route request baselines into the smart contract; the frontier premium share returns to the Collective for redistribution.*

© 2026 Happy Stack Calculus LLC &nbsp;·&nbsp; Draft for public comment
...
> | **Model/tooling** | Claude Opus 4.7, Gemini 3.1 Pro (Preview), GitHub Copilot in VS Code |
> | **Role** | human-directed drafting, citation lookup, LaTeX formatting, and revision support |
> | **Method** | multi-prompt iterative authoring with human-selected structure, argument, and final wording |
...
The attestation block appears in the acknowledgements section immediately before references in the typeset PDF version.
